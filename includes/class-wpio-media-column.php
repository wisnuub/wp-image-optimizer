...
class WPIO_Media_Column {
    ...
    public function render_column( $column_name, $attachment_id ) {
        ...
        // Check which formats have been converted.
        ...
        if ( $is_converted ) {
            ...
            $conv_size  = $best_conv_size;
            $saving_pct = $orig_size > 0 ? round( ( 1 - $conv_size / $orig_size ) * 100 ) : 0;
            $orig_kb    = round( $orig_size / 1024, 1 );
            $conv_kb    = round( $conv_size / 1024, 1 );

            // Badge(s)
            foreach ( $converted_formats as $fmt => $sz ) {
                echo '<span class="wpio-badge wpio-done">' . strtoupper( $fmt ) . ' ✓</span> ';
            }

            // Size comparison bar (show best format)
            ...
            echo   '<div class="wpio-size-row"><span class="wpio-size-lbl">' . strtoupper( $best_conv_label ) . '</span><span class="wpio-size-val">' . esc_html( $conv_kb ) . ' KB</span></div>';
