...
class WPIO_Admin {
                ...
                <?php if ( $format === 'webp' ) :
                    ...
                    <div class="wpio-alert info wpio-avif-hint" style="margin-top:14px;display:flex;align-items:center;gap:10px;flex-wrap:wrap;">
                        ...
                        <span style="flex:1;">
                            ...
                            Switch to <strong>AVIF</strong> for an extra ~20% file size reduction — unsupported browsers fall back to the original automatically.
                        </span>
                        <a href="#" class="wpio-notice-btn wpio-notice-btn-primary"
                           style="font-size:12px;padding:4px 12px;border-radius:4px;background:#FF2462;color:#fff;text-decoration:none;white-space:nowrap;"
    ...
    private function tab_advanced() {
        ...
        <div class="wpio-card">
            ...
            <div class="wpio-card-body">
                ...
                $folder_rows = array(
                    'uploads' => array(
                        'option' => 'wpio_scan_uploads',
                        'default' => '1',
                        'label'  => 'Uploads',
                        'path'   => '/wp-content/uploads/',
                        'desc'   => 'Default media library folder (always enabled)',
                        'locked' => true,
                    ),
                    'plugins' => array(
                        'option' => 'wpio_scan_plugins',
                        'default' => '0',
                        'label'  => 'Plugins',
                        'path'   => '/wp-content/plugins/',
                        'desc'   => 'Optimize bundled plugin images',
                    ),
                    'themes' => array(
                        'option' => 'wpio_scan_themes',
                        'default' => '0',
                        'label'  => 'Themes',
                        'path'   => '/wp-content/themes/',
                        'desc'   => 'Optimize theme images &amp; assets',
                    ),
                );
                foreach ( $folder_rows as $key => $row ) :
                    $counts = isset( $src_counts[ $key ] ) ? $src_counts[ $key ] : null;
                    $count_badge = '';
                    if ( $counts ) {
                        $count_badge = '<span style="display:inline-block;margin-left:8px;background:#f0f0f1;color:#555;'
                            . 'border-radius:20px;padding:1px 10px;font-size:11px;font-weight:600;vertical-align:middle;">';
                        $count_badge .= esc_html( $counts['total'] ) . ' images';
                        if ( $key === 'uploads' && $counts['thumbs'] > 0 ) {
                            $count_badge .= ' <span style="color:#999;font-weight:400;">(incl. ' . esc_html( $counts['thumbs'] ) . ' thumbnails)</span>';
                        }
                        $count_badge .= '</span>';
                    }
                ?>
                <div class="wpio-toggle-row">
                    ...
                    <label class="wpio-toggle">
                        ...
                        <input type="hidden" name="<?php echo esc_attr( $row['option'] ); ?>" value="1" />
                        <input type="checkbox" checked disabled />
                        <?php else : ?>
                        <input type="checkbox" name="<?php echo esc_attr( $row['option'] ); ?>" value="1"
                               <?php checked( get_option( $row['option'], $row['default'] ), '1' ); ?> />
                        <?php endif; ?>
                        <span class="wpio-toggle-slider"></span>
                    </label>
                </div>
                ...
                <div class="wpio-alert info" style="margin-top:8px;">
                    <span class="wpio-alert-icon">ℹ️</span>
                    <span>Scanning plugins or themes may take longer. Make sure PHP has write access.</span>
                </div>
            </div>
        </div>
        ...
        <div class="wpio-card">
            <div class="wpio-card-head"><div>
                ...
                <p>Directory name fragments separated by commas. Any path containing these strings will be skipped.</p>
            </div></div>
            <div class="wpio-card-body">
        ...
        <div class="wpio-card">
            <div class="wpio-card-head"><div>
                ...
                <p>Choose which PHP image library handles the conversion. Auto is recommended.</p>
            </div></div>
            <div class="wpio-card-body">
            ...
            <div class="wpio-card-body">
                <div class="wpio-toggle-row">
                    <div class="label">Skip if converted file is larger than original
                        <small>The converted file is discarded if it ends up bigger than the source</small>
                    </div>
        ...
        <div class="wpio-card" id="wpio-file-tree-card">
            <div class="wpio-card-head">
                ...
                <button type="button" class="wpio-btn wpio-btn-secondary" id="wpio-tree-refresh">🔄 Refresh</button>
            </div>
            <div class="wpio-card-body">
                <div id="wpio-tree-wrap">
                    <div id="wpio-tree-loading" style="color:#aaa;font-size:13px;padding:8px 0;">⏳ Loading file tree&hellip;</div>
        ...
        <!-- Server protection -->
        <div class="wpio-card">
            <div class="wpio-card-head"><div><h2>🛡️ Server protection <span class="wpio-expert-badge">Expert</span></h2><p>Keep bulk conversion gentle on shared hosting. Default values work well for most setups.</p></div></div>
            <div class="wpio-card-body">
                <div class="wpio-field-row">
    ...
    private function tab_delivery() {
        ...
        <div class="wpio-card">
            ...
            <div class="wpio-card-body">
                <div class="wpio-method-grid" style="grid-template-columns:repeat(3,1fr);">
                    <?php
                    $methods = array(
                        'rewrite' => array(
                            'name' => 'Server Rewrite',
                            'desc' => 'Apache .htaccess or Nginx rewrite rules. URLs stay the same — the server transparently serves the optimized file.',
                            'best' => 'Apache, LiteSpeed, self-managed Nginx',
                        ),
                        'html' => array(
                            'name' => 'HTML Rewriting',
                            'desc' => 'Rewrites <code>&lt;img&gt;</code> tags in page output to use <code>&lt;picture&gt;</code> elements with optimized sources.',
                            'best' => 'WP Engine, Kinsta, Flywheel, Pantheon, any managed host',
                        ),
                        'none' => array(
                            'name' => 'None (Manual)',
                            'desc' => 'Plugin only converts — you handle delivery yourself via CDN, theme code, or another plugin.',
                            'best' => 'Custom setups, CDN-level optimization',
                        ),
                    );
                ...
                <?php if ( $method === 'rewrite' ) : ?>
                    ...
                    <div class="wpio-alert warn" style="margin-top:14px;"><span class="wpio-alert-icon">⚠️</span><span>Nginx detected — .htaccess rules won't work. Paste the Nginx config below, or switch to <strong>HTML Rewriting</strong>.</span></div>
                    <?php else : ?>
                    <div class="wpio-alert ok" style="margin-top:14px;"><span class="wpio-alert-icon">✅</span><span>Apache/LiteSpeed detected — .htaccess rewrite rules are active automatically.</span></div>
                    <?php endif; ?>
                <?php elseif ( $method === 'html' ) : ?>
                    <div class="wpio-alert ok" style="margin-top:14px;"><span class="wpio-alert-icon">✅</span><span>HTML rewriting is active. Works on any hosting — no server configuration needed. Images are served via <code>&lt;picture&gt;</code> tags with automatic browser fallback.</span></div>
        ...
        <div class="wpio-card">
            ...
            <div class="wpio-card-body">
                <div class="wpio-alert info"><span class="wpio-alert-icon">ℹ️</span><span>
                    <?php if ( $method === 'html' ) : ?>
    ...
    public function ajax_queue_progress() {
        if ( ! check_ajax_referer('wpio_queue_progress','_wpnonce',false) || ! current_user_can('manage_options') ) wp_send_json_error('Unauthorized');
    ...
    public function ajax_save_settings() {
        ...
        foreach ( $all_options as $key => $type ) {
            ...
            // Uploads folder must always be enabled.
            if ( $key === 'wpio_scan_uploads' ) {
                $value = '1';
            }

            // Validate format values.
            if ( $key === 'wpio_format' && ! in_array( $value, array( 'webp', 'avif', 'both' ) ) ) {
                $value = 'webp';
            }
            if ( $key === 'wpio_delivery_method' && ! in_array( $value, array( 'rewrite', 'html', 'none' ) ) ) {
                $value = 'rewrite';
            }
